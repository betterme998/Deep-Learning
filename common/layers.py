...
class SoftmaxWithLoss: #Softmax 层和损失层
  def __init__(self):
    # 损失值，前向传播时计算
    self.loss = None
    # softmax 的输出（概率分布）
    self.y = None
    # 真实标签（教师数据），可能是 one-hot 向量或类别索引
    self.t = None

  def forward(self, x, t):
    """
    向前传播：计算 softmax 输出与交叉熵损失
    x: 神经网络的原始输出（得分），形状 (batch_size, num_classes)
    t: 真实标签，可以是 one-hot 编码 (batch_size, num_classes) 或类别索引 (batch_size,)
    """
    # 保存真实标签，供反向传播使用
    self.t = t
    # 计算 softmax：将得分转换为概率
  ...
  def backward(self,dout=1):
    """
    反向传播：计算损失对输入 x 的梯度
    dout: 从上游传来的梯度，这里通常为 1
    返回：dx，损失关于 x 的梯度
    """
    batch_size = self.t.shape[0]
     # 如果 t 是二维的 -> one-hot 向量
    if self.t.ndim == 2:
